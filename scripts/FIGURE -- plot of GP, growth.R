# figure 1 -- plots of GP & slope and int
	...
	## GP ~ intercept with phylo ##

	# load model that has GP ~ intercept (intercepts were standardized after first level)
	load(file = here("./output/MLM_GP_int_phylo_std.rds"))

	# extract posterior samples
	post_MLM_GP_int_phylo_std <- as.data.frame(MLM_GP_int_phylo_std)
	
	# all intercepts and slopes are reported in the output in terms of how different
	# the value is to the reference category, so need to add those to the reference
	# categories
	
	names(post_MLM_GP_int_phylo_std)
	beta_ints_phylo_std <- post_MLM_GP_int_phylo_std[69:100]
	...
	# change the column names to the species name
	names(beta_ints_phylo_std) <- species_list
	
	# put coefficients in a dataframe together for plotting
	
	# intercepts
	std_beta_ints_phylo_mean <- stack(apply(beta_ints_phylo_std, 2, mean)) %>%
											         rename(species = ind,
											 			          intercept = values)
	...
	std_beta_ints_phylo_mean <- merge(std_beta_ints_phylo_mean, GP_species)
	
	# plot data points
	GP_int_plot <- ggplot(data = std_beta_ints_phylo_mean, 
												aes(x = intercept, y = mean_gperf_simple)) +
								 geom_point()
	...
	# fitted line
	intercept_obs_phylo <- sort(std_beta_ints_phylo_mean$intercept)
	
	fitted_matrix_intercept_phylo <- t(sapply(1:nrow( as.data.frame(MLM_GP_int_phylo_std)), function(i) {
	                            		       post_MLM_GP_int_phylo_std[i, 'aGP_int'] +
	                            		       post_MLM_GP_int_phylo_std[i, 'bGP_int'] * intercept_obs_phylo
		}))		       
	
	mean_fitted_matrix_intercept_phylo <- colMeans(fitted_matrix_intercept_phylo)
	
	BCI_int_phylo <- t(apply(fitted_matrix_intercept_phylo, 2, quantile,
	                    prob = c(0.025, 0.975))) %>%
	            as.data.frame() %>%
	            rename(lower_BCI = "2.5%",
	                   upper_BCI = "97.5%") %>%
	            mutate(intercept_obs_phylo = intercept_obs_phylo,
	                   fitted_GP = mean_fitted_matrix_intercept_phylo)
	...
	# the value is to the reference category, so need to add those to the reference
	# categories
	names(post_MLM_GP_slope_phylo_std)
	beta_slope_phylo_std <-  post_MLM_GP_slope_phylo_std[69:100]
	
	# change the column names to the species name
	names(beta_slope_phylo_std) <- species_list
	
	# take mean for plotting
	std_beta_slope_phylo_mean <- stack(apply(beta_slope_phylo_std, 2, mean)) %>%
											         rename(species = ind,
											 			          slope = values)
	
	# get growth performance and add into dataset for plotting
	std_beta_slope_phylo_mean <- merge(std_beta_slope_phylo_mean, GP_species)
	
	# plot data points
	GP_slope_plot <- ggplot(data = std_beta_slope_phylo_mean, 
												aes(x = slope, y = mean_gperf_simple)) +
								 geom_point()
	...
	# fitted line
	slope_obs_phylo <- sort(std_beta_slope_phylo_mean$slope)
	
	fitted_matrix_slope_phylo <- t(sapply(1:nrow( as.data.frame(MLM_GP_slope_phylo_std)), function(i) {
	                            		       post_MLM_GP_slope_phylo_std[i, 'aGP_slope'] +
	                            		       post_MLM_GP_slope_phylo_std[i, 'bGP_slope'] * slope_obs_phylo
		}))		       
	
	mean_fitted_matrix_slope_phylo <- colMeans(fitted_matrix_slope_phylo)
	
	BCI_slope_phylo <- t(apply(fitted_matrix_slope_phylo, 2, quantile,
	                    prob = c(0.025, 0.975))) %>%
	            as.data.frame() %>%
	            rename(lower_BCI = "2.5%",
	                   upper_BCI = "97.5%") %>%
	            mutate(slope_obs_phylo = slope_obs_phylo,
	                   fitted_GP = mean_fitted_matrix_slope_phylo)
	...
	# intercept with phylo #

	GP_int_phylo <- 
		ggplot(BCI_int_phylo) +
		geom_ribbon(data = BCI_int_phylo, 
    	aes(x = intercept_obs_phylo,ymin = lower_BCI,
      ymax = upper_BCI), fill = "grey", alpha = 0.3) +
		geom_line(data = BCI_int_phylo, aes(x = intercept_obs_phylo, y = fitted_GP),
      color = "darkgrey", size = 1.25, alpha = 0.6) +
    geom_point(data = std_beta_ints_phylo_mean,
      aes(x = intercept, y = mean_gperf_simple),
      alpha = 0.6, size = 4) +
		 annotate("text", x = -2.7 , y = 5.4,
      label = "(c)", size = 5) +
	...
	GP_slope_phylo <- 
		ggplot(BCI_slope_phylo) +
		geom_ribbon(data = BCI_slope_phylo, 
  		aes(x = slope_obs_phylo,	ymin = lower_BCI,
  		ymax = upper_BCI),	fill = "grey", alpha = 0.3) +
  	geom_line(data = BCI_slope_phylo,
  		aes(x = slope_obs_phylo, y = fitted_GP),
  		color = "darkgrey", size = 1.25, alpha = 0.6) +
  	geom_point(data = std_beta_slope_phylo_mean,
  		aes(x = slope, y = mean_gperf_simple), alpha = 0.6, size = 4) +
		annotate("text", x = -2 , y = 5.4,
  		label = "(d)", size = 5) +
		scale_y_continuous(
  		name = "Growth performance",
  ...
  # create df of density of posterior to plot by hand
	...
	post_MLM_GP_int_phylo_std_slope <- post_MLM_GP_int_phylo_std_sum %>%
		filter(ind == "bGP_int")

	GP_int_dist_phylo <- 
		ggplot(data = post_MLM_GP_int_phylo_std, aes(bGP_int)) +
		geom_density(color = "black", fill = "grey50",  alpha = 0.5) +
		geom_segment(data = post_MLM_GP_int_phylo_std_slope,
			aes(x = LCI, xend = HCI, y = 0, yend = 0),
		  size = 0.5, color = "black") +
		 geom_point(data = post_MLM_GP_int_phylo_std_slope,
			aes(x = values, y = 0), size = 3) +
		annotate("text", x = post_MLM_GP_int_phylo_std_slope$values, y = 0.35,
		          label = post_MLM_GP_int_phylo_std_slope$values,
		          size = 3) +
		xlab("mean slope value") +
		scale_x_continuous(
			breaks = c(0, 1),
			labels = c(0, 1)) +
		theme_bw() +
	...
	GP_int_slope_phylo_prob0 <- (length(which(post_MLM_GP_int_std_phylo$bGP_int >0)))/length(post_MLM_GP_int_std_phylo$bGP_int) * 100
	GP_int_slope_phylo_prob0 <- round(GP_int_slope_phylo_prob0, 1)
	
	GP_int_phylo_dist <- 
		ggplot(data = post_MLM_GP_int_std_phylo, aes(bGP_int)) +
		geom_density(color = "black", fill = "grey50", alpha = 0.5)
	
	GP_int_phylo_dist_df <- ggplot_build(GP_int_phylo_dist)$data[[1]] 
	
	GSA_int_phylo_dist_base <- 
		ggplot() + 
		geom_area(data = subset(GP_int_phylo_dist_df, x < 0),
                         aes(x=x,y=y),
                         fill = "#cccccc",
                         color = "black", alpha = 0.5) +
		geom_area(data = subset(GP_int_phylo_dist_df, x > 0),
		...
		scale_x_continuous(
			...
			labels = c(0, 1)) +
		annotate("text", x = 0.38, y = 0.15,
		          label = paste0(GP_int_slope_phylo_prob0, ".0%"),
		          size = 3) +
		geom_segment(aes(x = -0.69, xend = 1.24, y = 0, yend = 0), color = "black") +
		theme_bw() +
		theme(
			axis.text.x = element_text(size = 6),
			axis.ticks.length.x = unit(-0.2, "cm"),
		...
		#ggplot_build(GSA_int_phylo_dist_base)$layout$panel_scales_x[[1]]$range$range

	# slope with phylo #
	
	post_MLM_GP_slope_phylo_std_slope <- post_MLM_GP_slope_phylo_std_sum %>%
		filter(ind == "bGP_slope")

	GP_slope_dist_phylo <- 
		ggplot(data = post_MLM_GP_slope_phylo_std, aes(bGP_slope)) +
    geom_density(color = "black", fill = "grey50",  alpha = 0.5) +
    geom_segment(data = post_MLM_GP_slope_phylo_std_slope,
    	aes(x = LCI, xend = HCI, y = 0, yend = 0),
      size = 0.3, color = "black") +
    geom_point(data = post_MLM_GP_slope_phylo_std_slope,
    	aes(x = values, y = 0), size = 3) +
    annotate("text", x = post_MLM_GP_slope_phylo_std_slope$values, y = 0.35,
              label = post_MLM_GP_slope_phylo_std_slope$values,
              size = 3) +
 	 xlab("mean slope value") +
  	scale_x_continuous(
			breaks = c(0, 1),
	...
	# create df of density of posterior to plot by hand
	post_MLM_GP_slope_std_phylo <- as.data.frame(MLM_GP_slope_phylo_std)
	GP_slope_slope_phylo_prob0 <- (length(which(post_MLM_GP_slope_std_phylo$bGP_slope >0)))/length(post_MLM_GP_slope_std_phylo$bGP_slope) * 100
	GP_slope_slope_phylo_prob0 <- round(GP_slope_slope_phylo_prob0, 1)
	
	GP_slope_phylo_dist <- 
		ggplot(data = post_MLM_GP_slope_std_phylo, aes(bGP_slope)) +
		geom_density(color = "black", fill = "grey50", alpha = 0.5)
	
	GP_slope_phylo_dist_df <- ggplot_build(GP_slope_phylo_dist)$data[[1]] 
	
	GSA_slope_phylo_dist_base <- 
		ggplot() + 
		geom_area(data = subset(GP_slope_phylo_dist_df, x < 0),
                         aes(x=x,y=y),
                         fill = "#cccccc",
                         color = "black", alpha = 0.5) +
		geom_area(data = subset(GP_slope_phylo_dist_df, x > 0),
		...
		scale_x_continuous(
			...
			labels = c(0, 1)) +
		annotate("text", x = 0.38, y = 0.15,
		          label = paste0(GP_slope_slope_phylo_prob0, "%"),
		          size = 3) +
		geom_segment(aes(x = -0.81, xend = 1.38, y = 0, yend = 0), color = "black") +
		theme_bw() +
		theme(
			axis.text.x = element_text(size = 6),
			axis.ticks.length.x = unit(-0.2, "cm"),
	...
	#ggplot_build(GSA_slope_phylo_dist_base)$layout$panel_scales_x[[1]]$range$range
	...
	plot5 <- GP_int_phylo +
					 theme(plot.margin = unit(c(-0.05, 0, 0.2, 0.2), "in"))				 
	
	plot6 <- GP_slope_phylo +
					 theme(plot.margin = unit(c(-0.05, 0.2, 0, -0.05), "in"))

	plot7 <- GSA_int_phylo_dist_base +
	 theme(
		panel.background = element_rect(fill = "transparent"),
    plot.background = element_rect(fill = "transparent", color = NA),
		panel.border = element_blank())

	plot8 <- GSA_slope_phylo_dist_base + 
	 theme(
		panel.background = element_rect(fill = "transparent"),
    plot.background = element_rect(fill = "transparent", color = NA),
		panel.border = element_blank())
	
	plot5_inset <- plot5 + inset_element(plot7, 0.7, 0.005, 1, 0.3, clip = TRUE)

	plot6_inset <- plot6 + inset_element(plot8,0.7, 0.005, 1, 0.3, clip = TRUE)

	
	Fig_top <- plot1_inset + plot2_inset + plot_layout(ncol = 2, widths = c(1, 1))

	Fig_bot <- plot5_inset + plot6_inset + plot_layout(ncol = 2, widths = c(1, 1))

	Fig_GSA_GP <- Fig_top/Fig_bot

	ggsave(Fig_GSA_GP, 
			 file = here("./output/plots/Fig_GSA_GP.png"),
									 height = 7.5, width = 12, units = "in")


	# add in image
	library(magick)
	phylo <- image_read("./data/phylo.png")

	Fig_GSA_GP_images <- ggdraw() +
                  draw_plot(Fig_GSA_GP) +
                  draw_image(phylo, x = 0.035, y = 0.1,
                             width = 0.085, height = 0.065) +
									draw_image(phylo, x = 0.5, y = 0.1,
                             width = 0.085, height = 0.065)

	ggsave(Fig_GSA_GP_images, file = here("./output/plots/Fig_GSA_GP_images.png"),
       height = 7.5, width = 12, units = "in")

	Fig_GSA_GP_images_test <-	Fig_GSA_GP_images +
		annotate("text", x = 0.128, y = 0.13,
              label = "+ phylo",
              size = 5) +
		annotate("text", x = 0.595, y = 0.13,
              label = "+ phylo",
              size = 5) 

	ggsave(Fig_GSA_GP_images_test, file = here("./output/plots/Fig_GSA_GP_images_test.png"),
       height = 7.5, width = 12, units = "in")
